...
var colorCount = 256;
var names = ["Diverging", "Rainbow", "GreyRainbow", "RainbowInv", "SeisSol", "Kaikoura", "PGA", "PGV", "PGD", "large"];
var scales = {
...
var isMapPga = mapName.localeCompare(names[6]) == 0, isMapPgv = mapName.localeCompare(names[7]) == 0,
    isMapPgd = mapName.localeCompare(names[8]) == 0, isMapLarge = mapName.localeCompare(names[9]) == 0;
...
var baseMaps = {
    ...
    [names[9]]: L.tileLayer('http://localhost/large/{z}/{x}/{y}.png', {maxZoom: 18})
};
var map = L.map('mapid', {
...
map.on('baselayerchange', function(e) {
    ...
    isMapLarge = mapName.localeCompare(names[9]) == 0;
    
    if (isMapPga || isMapPgv || isMapPgd || isMapLarge) colorScaleName = names[0];
...
map.on('zoomend', function() {
    for (var i = 0; i < sets.length; i++) {
        ...
        isSetLarge = sets[i].name.indexOf(names[9]) == 0;
...
function processColor(set, index) {
    ...
    isSetLarge = set.name.indexOf(names[9]) == 0;
    if (!markers[index]) {
